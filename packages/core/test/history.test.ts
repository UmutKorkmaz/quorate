...
import { computeStats, dedupeHistoryEntries, isHistoryEntry, toHistoryEntry } from "../src/history.js";
...
function report(
  findings: Finding[],
  overrides: Partial<CouncilReport> & { metadata?: Partial<CouncilReport["metadata"]> } = {},
...
describe("computeStats", () => {
  const entries = [
    toHistoryEntry(report([finding({ severity: "high", file: "a.ts" })], { verdict: "fail", metadata: { reviewId: "r1" } })),
    toHistoryEntry(report([finding({ severity: "medium", file: "a.ts" })], { verdict: "warn", metadata: { reviewId: "r2" } })),
    toHistoryEntry(report([], { verdict: "pass", metadata: { reviewId: "r3" } }))
  ];
  ...
  it("dedupes repeated review ids by keeping the newest run", () => {
    const older = toHistoryEntry(report([finding()], { verdict: "fail", metadata: { reviewId: "same", generatedAt: "2026-06-15T00:00:00.000Z" } }));
    const newer = toHistoryEntry(report([], { verdict: "pass", metadata: { reviewId: "same", generatedAt: "2026-06-16T00:00:00.000Z" } }));
    expect(dedupeHistoryEntries([older, newer])).toEqual([newer]);
    expect(computeStats([older, newer]).runs).toBe(1);
  });

  it("validates history entry shape strictly", () => {
    const valid = toHistoryEntry(report([]));
    expect(isHistoryEntry(valid)).toBe(true);
    expect(isHistoryEntry({ ...valid, verdict: "ok" })).toBe(false);
    expect(isHistoryEntry({ ...valid, findingCounts: { severe: 1 } })).toBe(false);
  });
});
